...
One of the greatest things about `R` is the vibrant community supporting it. One can find endless tutorials, books, presentations, etc. on just about any aspect of `R` you want -- from beginner topics to advances matters. Once you have some familiarity with `R`, you'll be able to "Google your way out" of most problems you run into. Here's a small list of useful resources:
...
`R` was developed as an *interactive* computing language. This means users enter commands into the machine line-by-line to produce a result. `R`'s original purpose was to allow researchers at Bell Labs interact with and analyze their data. The `R` console can be accessed in a number of ways:

From a command prompt/terminal:

![cmd-term](images/old-r-console.png)

From the `R` GUI distribution downloaded with `R`:

![gui-term](images/r-gui.png)

And, of course, from RStudio:
...
Packages are what make `R` so great. A package is 
...
Perhaps in it's most basic form, `R` can be used as a glorified calculator. Enter the following commands into your `R` console or send them to the console from a script with CMD+Enter on OS X or Ctrl+Enter on Windows.
