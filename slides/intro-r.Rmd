...
* First and foremost, a fully functional programming language
...
* Lists are containers allowing to store multiple *R* objects in the same parent object 
* Unlike vectors, enclosed data can have different *types* 
* As an example, we'll create a list with some info about a person:

```
> person <- list(first = 'andrew', last = 'borgman', age = 25)
> person
$first
[1] "andrew"
...
* Pre-*RStudio*, all of this searching took place on the command line
* You may find this is a more convenient way to search for *R* help
* The `help()` and `?` functions can be used to search the documentation for functions matching the inputted text
* The following functions both search *R*'s help documentation for any functions named `mean`
...
* But sometimes we don't know the name of the function we seek
* Fortunately, *R*'s documentation search is flexible enough to accommodate that 
* Let's try to find a function that lets us compute the standard deviation of a vector of numbers. 
* First, we'll try a search with the name of the function used in *Excel* software
...
* You'll see that *R* gives us a hint and is telling us to use the function `??`. 
* This function works like the search box we used to find help in *RStudio*. Let's search for "standard deviation":
...
* The `rm()` command allows you to selectively remove objects from your *R* session when they are no longer needed 
...
```{r, eval = T, results='markup', fig.show='asis'}
...
```{r, eval = T, results='markup', fig.show='asis', fig.height = 4, fig.width = 4.5}
qplot(x = Gene, y = Expression, data = gene.exprs.long, geom = 'boxplot', fill = Gene)
```

## Or Make Some Facets

```{r, eval = T, results='markup', fig.show='asis', fig.height = 4, fig.width = 4.5}
qplot(x = Group, y = Expression, data = gene.exprs.long, fill = Group, facets = ~ Gene, geom = 'boxplot')
```

## Or Make Some Facets & Make It Pretty

```{r, eval = T, results='markup', fig.show='asis', fig.height = 4, fig.width = 4.5}
qplot(x = Group, y = Expression, data = gene.exprs.long, color = Group, facets = ~ Gene, geom = 'jitter') + theme_bw()
```
